...
| **CLI `pull`** | Done | `envo pull --project X --env Y` uses the personal vault by default, writes `.env`, and auto-adds `.env` to `.gitignore` |
| **CLI `run`** | Done | `envo run --project X --env Y -- npm start` injects secrets into a child process without writing them to disk |
...
| **Vercel Sync** | Done | Encrypted personal platform connections + manual sync from the environment page or `envo sync` |
...
### What's NOT Built Yet (from the [product roadmap](roadmap.md))
...
- Per-project or per-environment permissions
- SSO / SAML
...
Envo now implements this bridge for Vercel: users can store an encrypted Vercel token and trigger a manual sync from the environment page or with `envo sync`. Railway, Render, and Fly.io still require adapters.
...
| Vercel environment sync | **Works now** | Encrypted connection + web and CLI sync |
| `envo sync --target railway` | **Not yet** | Push secrets to platform APIs |
| Platform connection management | **Partial** | Vercel connections are available and currently user-owned |
...
3. **Later**: Platform sync adapters for Railway, Render, and Fly
...
| Task | Scope | Effort |
|------|-------|--------|
| Auto-resolve personal workspace when `--org` omitted | Shipped for `pull`, `run`, and `sync` | Done |
...
| Task | Scope | Effort |
|------|-------|--------|
| `envo sync` framework | Shipped with Vercel support | Done |
| Railway adapter | Backend platform service | Medium |
| Render adapter | CLI `internal/platforms/render.go` | Small |
| Vercel adapter | Shipped in the backend sync service | Done |
| Fly.io adapter | CLI `internal/platforms/fly.go` | Small |
| Platform connection management in frontend | Shipped in Settings for Vercel | Done |
